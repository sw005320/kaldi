...
stat=`find $wsj_multi -type d -name stat` 
for y in `seq 6 10`; do
    find $stat -name T$y | xargs cp -r -t $reverb_real_dt/audio/stat
done
for z in `seq 21 25` `seq 36 40`; do
    find $stat -name T$z | xargs cp -r -t $reverb_real_et/audio/stat
done
find $wsj_multi -name sentencelocation -print | xargs cp -r -t $reverb_real_dt/etc
find $wsj_multi -name WSJ.mlf | xargs cp -r -t $reverb_real_dt/mlf
find $wsj_multi -name WSJ.mlf | xargs cp -r -t $reverb_real_et/mlf

# Convert .flac file to .wav file
echo "Convert .flac file to .wav file."
echo "Log file is in $dir/flac.log"
flac -d -f --delete-input-file {$reverb_real_dt,$reverb_real_et}/audio/stat/T*/*/*/*.flac > $dir/flac.log 2>&1
